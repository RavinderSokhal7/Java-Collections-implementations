...
class MyHashSet {
    ...
    private Node insert(Node root, int key, int value){
    	...
    	if(root == null) return new Node(hash, key, value);
    	Node p = root;
    	while(p!=null){
    		if(p.key == key) return root;
    		else if(p.key>key){
    			if(p.left==null){
    				p.left = new Node (hash, key, value);return root;
    			}
    			p = p.left;
    		}
    		else{
    			if(p.right==null){
    				p.right = new Node(hash, key, value); return root;
    			}
    			p = p.right;
    		}
    	}
    ...
    //Find in BST
    private Node find(Node p, int key){
    	if(p==null) return p;
    	while(p!=null){
    ...
    public void add(int key, int value) {
        int hash = hashCode(key);
        HashT[hash] = insert(HashT[hash], key, value);
    }
    ...
    public boolean contains(int key) {
        return find(HashT[hashCode(key)],key)==null;
    }
    // get value of key
    public int get(int key){
    	Node p = find(HashT[hashCode(key)],key);
